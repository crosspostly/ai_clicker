...
class Validator {
  ...
  static validateInstructions(instructions) {
    ...
    if (instructions.length > 5000) {
      throw new ValidationError('Instructions text is too long (max 5000 chars)');
    }
    return true;
  }
  ...
  static validateRange(value, min, max) {
    ...
    if (num < min || num > max) {
      throw new ValidationError(`Value must be between ${min} and ${max}, got: ${num}`);
    }
    return true;
  }
  ...
  static validateDuration(duration) {
    if (typeof duration !== 'number' || duration < 0 || duration > 300000) {
      throw new ValidationError('Duration must be a number between 0 and 300000ms');
    }
    return true;
  }
}
